...
router.get('/analytics', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const [total, todayCount, pending, completed, revToday, revMonth, pendingPaymentsResult, Employee] = await Promise.all([
            ...
            Order_1.Order.countDocuments({ createdAt: { $gte: today } }),
            Order_1.Order.countDocuments({ status: { $ne: 'Delivered' } }),
            Order_1.Order.countDocuments({ status: 'Delivered' }),
            Order_1.Order.aggregate([{ $match: { createdAt: { $gte: today } } }, { $group: { _id: null, total: { $sum: '$totalAmount' } } }]),
            Order_1.Order.aggregate([{ $match: { createdAt: { $gte: startOfMonth } } }, { $group: { _id: null, total: { $sum: '$totalAmount' } } }]),
            Order_1.Order.aggregate([
                { $project: { due: { $subtract: ['$totalAmount', { $ifNull: ['$paidAmount', 0] }] } } },
                { $match: { due: { $gt: 0 } } },
                { $group: { _id: null, totalDue: { $sum: '$due' } } }
            ]),
            mongoose_1.default.model('Employee').countDocuments({ status: 'Active' }) // Assuming Employee has status Active
        ]);
        // Service Breakdown
        const serviceStats = await Order_1.Order.aggregate([
        ...
        const dailyRevData = await Order_1.Order.aggregate([
        ...
        const monthlyRevData = await Order_1.Order.aggregate([
...
router.get('/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        const order = await Order_1.Order.findById(req.params.id).populate('customer');
        if (!order)
...
router.get('/customer/:customerId', auth_1.authenticateToken, async (req, res) => {
    try {
        const orders = await Order_1.Order.find({ customer: req.params.customerId }).sort({ createdAt: -1 });
        res.json(orders);
    }
    catch (error) {
        res.status(500).json({ message: 'Error fetching orders' });
...
// POST new order
router.post('/', auth_1.authenticateToken, validators_1.orderValidators, validators_1.validateRequest, async (req, res) => {
    try {
        const orderData = req.body;
        const order = new Order_1.Order(orderData);
        const savedOrder = await order.save();
        // Update customer stats
        await Customer_1.Customer.findByIdAndUpdate(orderData.customer, {
...
// PUT update payment
router.put('/:id/payment', auth_1.authenticateToken, async (req, res) => {
    try {
